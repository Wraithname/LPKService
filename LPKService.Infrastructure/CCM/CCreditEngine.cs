    ...
    public class CCreditEngine:CCMRepoBase
    {
        ...
        private int GetId()
        {
            ...
            try
            {
                ...
                string sqlstr = $"SELECT CUSTOMER_ID FROM customer_catalog_credit Order by CUSTOMER_ID";
                using (OracleConnection connection = BaseRepo.GetDBConnection())
                {
        ...
        {
            catCredit.customerId = 1 + GetId();
            bool res = false;
                try
                {
                    OracleDynamicParameters odp = new OracleDynamicParameters();
                    string sqlstr = "INSERT INTO CUSTOMER_CATALOG_CREDIT (CUSTOMER_ID,CREDIT_STATUS,ADDRESS_ID,DB_NUMBER," +
